...
slab = Asym_slab(c0=1.3, R1=5/9, R2=5/6, K=None, M_A=1.4)
...
line1 = sol.line_trace_sp(slab.disp_rel, 1, 1.262, 0.01, 0.14, 4, args=(slab.K, slab.M_A))
line2 = sol.line_trace_mp(slab.disp_rel_mp, 2, 0.786 + 0.05 * 1j, 0.01, 0, 4, args=(slab.K, slab.M_A))
line3 = sol.line_trace_sp(slab.disp_rel, 1, 0.643, 0.01, 0.05, 4, args=(slab.K, slab.M_A))
line4 = sol.line_trace_sp(slab.disp_rel, 1, 0.479, 0.01, 0.05, 4, args=(slab.K, slab.M_A))
line5 = sol.line_trace_sp(slab.disp_rel, 2, 0.558, 0.01, 0.05, 4, args=(slab.K, slab.M_A))
line6 = sol.line_trace_sp(slab.disp_rel, 1, -1.157, 0.01, 0.3, 4, args=(slab.K, slab.M_A))
line7 = sol.line_trace_sp(slab.disp_rel, 2, -1.294, 0.01, 1.25, 4, args=(slab.K, slab.M_A))
line8 = sol.line_trace_sp(slab.disp_rel, 3, -1.356, 0.01, 2.15, 4, args=(slab.K, slab.M_A))
line9 = sol.line_trace_sp(slab.disp_rel, 3.6, -1.564, 0.01, 3.06, 4, args=(slab.K, slab.M_A))

ax.plot(line1[:,0], line1[:,1], color = 'b', linewidth = 2, linestyle='-', label='Stable')
ax.plot(line2[:,0], line2[:,1], color = 'b', linewidth = 2, linestyle='--', label='Unstable')
ax.plot(line2[:,0], np.imag(line2[:,1]), color = 'r', linewidth = 2, linestyle='-', label='Imaginary')
ax.plot(line2[:,0], -np.imag(line2[:,1]), color = 'r', linewidth = 2, linestyle='-')
...
ax.fill_between(x_range, 1+slab.M_A, slab.cT+slab.M_A, color='0.9')
ax.fill_between(x_range, slab.c0 + slab.M_A, 1+slab.M_A, color='1', hatch='/', edgecolor='0')
ax.fill_between(x_range, np.minimum(slab.c1, slab.c2), slab.c0 + slab.M_A, color='1', hatch='/')

plt.legend(loc=4)

plt.savefig('images_traced/evacuated6.png', bbox_inches='tight')