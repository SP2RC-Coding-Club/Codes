...
line1 = sol.line_trace_sp(slab.disp_rel, 0.15, 0.215, 0.002, 0, 4, args=(slab.K, slab.M_A))
...
break_pt = int(np.where(np.array([line101[i+1] - line101[i]
                                  for i in np.arange(len(line101)-1)]) > 0.1)[0])

line2 = sol.line_trace_sp(slab.disp_rel, 0.56, -0.088, 0.01, 0, 4, args=(slab.K, slab.M_A))
line3 = sol.line_trace_sp(slab.disp_rel, 0.1, -0.843, 0.01, 0, 4, args=(slab.K, slab.M_A))
line4 = sol.line_trace_sp(slab.disp_rel, 2.48, -0.658, 0.01, 1.3, 4, args=(slab.K, slab.M_A))
line5 = sol.line_trace_sp(slab.disp_rel, 0.44, 0.77, 0.01, 0, 4, args=(slab.K, slab.M_A))
line6 = sol.line_trace_sp(slab.disp_rel, 0.27, 0.381, 0.002, 0.06, 0.325, args=(slab.K, slab.M_A))
line7 = sol.line_trace_sp(slab.disp_rel, 2.4, 0.343, 0.005, 0.125, 4, args=(slab.K, slab.M_A))
line8 = sol.line_trace_sp(slab.disp_rel, 0.54, 0.392, 0.01, 0, 4, args=(slab.K, slab.M_A))
line9 = sol.line_trace_sp(slab.disp_rel, 2.85, 0.363, 0.01, 0, 4, args=(slab.K, slab.M_A))
line10 = sol.line_trace_sp(slab.disp_rel, 0.54, 0.403, 0.002, 0.51, 4, args=(slab.K, slab.M_A))

ax.plot(line101[:break_pt], line111[:break_pt],	color = 'b', linewidth = 2, linestyle='-', label='Stable')
ax.plot(line101[break_pt+1:], line111[break_pt+1:],	color = 'b', linewidth = 2, linestyle='-')
ax.plot(line102, line112, color = 'b', linewidth = 3, linestyle='--', label='Unstable')
ax.plot(line101[:break_pt], np.imag(line111[:break_pt]), color = 'r', linewidth = 2, linestyle='-')
ax.plot(line101[break_pt+1:], np.imag(line111[break_pt+1:]), color = 'r', linewidth = 2, linestyle='-')
ax.plot(line102, np.imag(line112), color = 'r', linewidth = 2, linestyle='-', label='Imaginary')
ax.plot(line102, -np.imag(line112), color = 'r', linewidth = 2, linestyle='-')
...
plt.savefig('images_traced/dense5.png', bbox_inches='tight')