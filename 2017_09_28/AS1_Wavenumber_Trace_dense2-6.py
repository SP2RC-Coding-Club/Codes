...
slab = Asym_slab(c0=0.6, R1=5/3, R2=2.5, K=None, M_A=0.9)
...
ax.plot(line1[:,0][np.abs(np.imag(line1[:,1])) < 1e-5],
        np.imag(line1[:,1][np.abs(np.imag(line1[:,1])) < 1e-5]),
        color = 'r', linewidth = 2, linestyle='-', label='Imaginary')
ax.plot(line1[:,0][np.abs(np.imag(line1[:,1])) > 1e-5],
        np.imag(line1[:,1][np.abs(np.imag(line1[:,1])) > 1e-5]),
        line1[:,0][np.abs(np.imag(line1[:,1])) > 1e-5],
        -np.imag(line1[:,1][np.abs(np.imag(line1[:,1])) > 1e-5]),
...
plt.savefig('images_traced/dense6.png', bbox_inches='tight')