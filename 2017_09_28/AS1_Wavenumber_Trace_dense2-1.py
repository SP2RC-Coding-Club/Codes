...
slab = Asym_slab(c0=0.6, R1=5/3, R2=5/3, K=None, M_A=0.4)
...
line1 = sol.line_trace_sp(slab.disp_rel, 1, 0.666, 0.01, 0, 4, args=(slab.K, slab.M_A))
line2 = sol.line_trace_sp(slab.disp_rel, 1, 0.572, 0.01, 0, 4, args=(slab.K, slab.M_A))
line3 = sol.line_trace_sp(slab.disp_rel, 1, -0.41, 0.01, 0, 4, args=(slab.K, slab.M_A))
line4 = sol.line_trace_sp(slab.disp_rel, 1, -0.699, 0.01, 0, 4, args=(slab.K, slab.M_A))
line5 = sol.line_trace_sp(slab.disp_rel, 3, -0.823, 0.01, 2.57, 4, args=(slab.K, slab.M_A))
line6 = sol.line_trace_sp(slab.disp_rel, 2.4, -0.161, 0.001, 0.06, 4, args=(slab.K, slab.M_A))
line7 = sol.line_trace_sp(slab.disp_rel, 2.24, -0.12, 0.002, 0.36, 4, args=(slab.K, slab.M_A))
...
ax.fill_between(x_range, np.max(-slab.c1, -slab.c2), -1 + slab.M_A, color='0.9')
ax.fill_between(x_range, np.max(-slab.c1, -slab.c2), y_range[0], color='1', hatch='/', edgecolor='0')
ax.fill_between(x_range, np.min(slab.c1, slab.c2), y_range[-1], color='1', hatch='/', edgecolor='0')

plt.legend(loc=1)

plt.savefig('images_traced/dense1.png', bbox_inches='tight')